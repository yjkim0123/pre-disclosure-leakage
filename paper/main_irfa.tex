...
\begin{abstract}
We test whether South Korea's Fair Disclosure regulation effectively prevents directional information leakage before earnings announcements. Using 2,221 disclosure filings from 64 KOSPI-listed companies across 12 chaebol groups (2020--2026), we find that pre-disclosure abnormal returns, while significant under parametric tests (CAR[-5,$-$1] = +0.52\%, $p$<0.001), do not predict earnings surprise direction (49.0\% accuracy, $p$=0.649). Non-parametric tests confirm the drift is driven by outliers (sign test $p$=0.585; Wilcoxon $p$=0.142), revealing a divergence between parametric and non-parametric inference. In contrast, the first post-disclosure trading day responds correctly: positive surprises generate AR of +0.55\%, negative surprises $-$0.58\% ($p$<0.001), with 60.6\% direction accuracy ($p$<0.001). We document that 96.5\% of filings occur after market close, making Day~0 effectively pre-disclosure. Cross-sectional analyses reveal heterogeneous drift across chaebol groups, firm sizes, and market betas, but no pattern consistent with differential information leakage ($R^2$=0.002). Our findings suggest Korea's Fair Disclosure regime effectively prevents directional leakage, even as non-directional price noise emerges pre-disclosure.
\end{abstract}
...
In Korea, \citet{yun2012fair} examined the effect of fair disclosure regulation on the timeliness and informativeness of earnings announcements, while \citet{oh2017disclosure} examined how credit ratings interact with corporate disclosure behavior under Korea's fair disclosure regulation. More recently, \citet{shin2023korea} study the effects of compulsory preliminary earnings announcements on stock prices, and \citet{lee2019regulation} examine the interaction between R\&D intensity and fair disclosure in Korea, finding that disclosure effects vary by firm characteristics. However, these studies do not examine whether pre-disclosure price movements are directionally informative with respect to the actual earnings news---the central question of our study.
...
We employ year-over-year operating profit changes as our surprise measure. While analyst consensus forecasts (e.g., I/B/E/S) are standard in U.S. studies \citep{livnat2006comparing}, comprehensive consensus data for Korean firms require commercial subscriptions (e.g., FnGuide, WiseFn) that are unavailable to independent researchers without institutional access. The YoY measure provides a conservative test: if directional leakage existed, it would manifest as pre-disclosure returns aligned with the direction of YoY changes, which we do not observe. Moreover, YoY operating profit changes capture the directional content of earnings news without relying on potentially stale or biased analyst expectations.
...
We observe a structural shift in event-day CARs: positive during 2020--2022 (COVID-19 recovery) and negative during 2024--2026 (Fig.~\ref{fig:year}). A significant quarterly pattern emerges (Fig.~\ref{fig:quarter}), with Q1 disclosures generating negative CARs ($-$0.51\%, $p$=0.010) and Q2 generating positive CARs (+0.60\%, $p$=0.003). Cross-sectional OLS of CAR[-3,$-$1] on log(size) and beta yields $R^2=0.002$, confirming that firm characteristics explain virtually none of the pre-disclosure drift.

\paragraph{Selection bias assessment.}

Our parseable earnings surprise subsample covers 26.4\% of all events (584 of 2,208), raising a potential selection concern. To assess whether this subsample is systematically different, we compare pre-disclosure CARs between parseable and non-parseable events. The two subsamples show statistically indistinguishable pre-disclosure CARs: parseable events exhibit CAR[-5,$-$1] of +0.32\% versus +0.59\% for non-parseable events ($t$=1.45, $p$=0.147). Importantly, the parseable subsample represents the largest firms with the most analyst coverage---precisely the firms where information leakage would be most valuable and most likely to occur if present. If pre-disclosure directional leakage existed, it would be most detectable among these high-visibility firms. The absence of directional drift in this subsample therefore constitutes a conservative test: finding no evidence of leakage among the firms with the greatest leakage incentives strengthens, rather than weakens, the conclusion that Korea's Fair Disclosure regime is effective. Fig.~\ref{fig:distributions} presents the distribution of pre-disclosure CARs for the full sample, illustrating the right-skewed nature of returns that drives the divergence between parametric and non-parametric test results.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig4_quarter.pdf}
\caption{Pre-disclosure CAR[-3,$-$1] and event-day CAR[0,+1] by fiscal quarter. Q1 disclosures generate negative event-day CARs ($-$0.51\%, $p$=0.010), while Q2 generates positive CARs (+0.60\%, $p$=0.003).}
\label{fig:quarter}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig5_distributions.pdf}
\caption{Distribution of pre-disclosure CAR[-5,$-$1] across all events ($N$=2,208). The right-skewed distribution explains the divergence between parametric (mean-based) and non-parametric (median-based) test results.}
...
Several limitations warrant discussion. First, our YoY operating profit change as a surprise measure lacks the precision of analyst consensus forecasts \citep{livnat2006comparing}. Only 26.4\% of events yield parseable surprises, though as discussed in Section~\ref{sec:results}, this subsample's composition (larger, more-scrutinized firms) strengthens rather than weakens the conservative test interpretation. Second, our sample is limited to 64 large chaebol affiliates; information leakage may be more prevalent among smaller, less scrutinized firms. Third, Korean Fama-French factors are unavailable, preventing multi-factor benchmark models. Fourth, we cannot rule out that pre-disclosure volume increases reflect legal mosaic information assembly. Fifth, our analysis focuses on earnings disclosures and does not examine other material events where leakage incentives may differ.
